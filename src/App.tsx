...
export default function App() {
  const [items, setItems] = useState<Referral[]>(
    () => referralsFixture as unknown as Referral[]
  );
      ...
      try {
        ...
        const updated = {
          ...
          documents: active.documents.map((d) =>
            d.id === pdfDoc.id
              ? { ...d, text, confidence: Math.max(d.confidence ?? 0, 0.98) }
              : d
          ),
        };
        setItems((prev) =>
          prev.map((r) => (r.id === updated.id ? (updated as Referral) : r))
        );
      } catch (e) {
    ...
    <div className="container">
        ...
        <div className="brand">
          ...
          <div>
            ...
            <div className="tagline">
              Receiver-side, rural-first triage • AI at the center
            </div>
          </div>
        </div>
        <div className="toolbar">
          <div className="pill">Demo mode · fixtures</div>
          <button className="button ghost" onClick={() => setUseLLM((v) => !v)}>
            {useLLM ? "ChatGPT: ON" : "ChatGPT: OFF"}
          </button>
          <div
            className="pill"
            style={{ backgroundColor: "#fef3c7", color: "#92400e" }}
          >
            OpenAI API: To be implemented
          </div>
          <button
            className="button ghost"
            onClick={() => setLowConnectivity((v) => !v)}
          >
            {lowConnectivity ? "Low Connectivity: ON" : "Low Connectivity: OFF"}
          </button>
          {queue.length > 0 && (
            <button className="button warn" onClick={flushQueue}>
              Process {queue.length} queued
            </button>
          )}
          <a
          ...
          <div className="card">
            ...
            <div className="section queue">
              {queue.length ? (
                queue.map((q) => (
                  <div key={q.id}>
                    • {new Date(q.ts).toLocaleTimeString()} — {q.action}
                  </div>
                ))
              ) : (
      ...
      >
        ...
        <span className="muted" style={{ fontSize: 13 }}>
          This is a mocked, client-only demo. Replace fixtures with live rails
          (Stedi/Twilio) when ready.
        </span>
        <div
          style={{
            padding: "4px 8px",
        ...
        >
          ⚠️ OpenAI API integration pending
        </div>
      </div>
    </div>
  );
